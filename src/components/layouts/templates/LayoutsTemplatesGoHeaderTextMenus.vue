<template>
  <ul class="menu menu-horizontal p-1 rounded-box">
    ...
    >
      <!-- submenuがない場合 -->
      <LayoutsElementsHeaderTextMenu
        v-if="!menu.pullDowns.length"
        :menu="menu"
      />

      <!-- submenuがある場合 -->
      <LayoutsElementsHeaderPullDownMenu
        v-else
        :menu="menu"
      />
    </template>
...
<script lang="ts" setup>
  import items from "~~/configs/components/templates/layoutsTemplatesGoHeaderTextMenus/go";
